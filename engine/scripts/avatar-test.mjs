...
section('钳制与默认（distance 1..2000 默认 10；degrees -180..180）');
{
  ...
  const jumpQ = (t) => parseAvatarCommand(t)[0].kind === 'jump';
  ok(jumpQ('跳10 下'), '「跳10 下」→ jump（量词容错）');
  ok(jumpQ('跳十下'), '「跳十下」→ jump（中文数字）');
  ok(jumpQ('跳 3 下'), '「跳 3 下」→ jump');
  ok(jumpQ('跳一下吧'), '「跳一下吧」→ jump');
